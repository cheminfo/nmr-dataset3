...
const dirs = readdirSync(DATA_DIR)
  ...
  .filter((dir) => statSync(join(DATA_DIR, dir)).isDirectory());
...
for (const dir of dirs) {
  ...
  let molfiles = readdirSync(join(DATA_DIR, dir)).filter((filename) =>
  ...
  let jcamps = readdirSync(join(DATA_DIR, dir)).filter((filename) =>
    filename.endsWith("dx")
  );
  json.spectra[0].source.jcampURL = URL_FOLDER + "/" + jcamps[0];
